...
**nmap -PR (ARP Scan): روش خوبیه و مطمعنه چون اکثر فایروال ها درخواست arp رو بلاک نمیکنن ! اما نمیدونم چرا گوشیم رو پیدا نکرد ! 
**nmap -PU (UDP Scan) :برای اینترنت استفاده میشه چون اگه بخوایم توی شبکه لن استفادش کنیم متود بهتری برای اسکن داریم که همون متود قبلیه ! این متود یه پکت یو دی پی خالی رو برای یه پورت غیرمعقول که احتمال زیاد بسته هست میفرسته و سیستم پکت رو میگیره و پیام پورت در دسترس نیست رو برمیگردونه ! اگه جوابمون سکوت باشه ممکنه فایروال جلوی پکت رو گرفته باشه ا

**nmap -PE (ICMP Ping Echo Scan ) : تقریبا همون پینگ معموله و توی شبکه داخلی بهتره چون که مهدسای شبکه برای بررسی سیستم هاشون معمولا پینگ رو نمیبندن اما برای شبکه خارجی زیاد مناسب نیست ! و اینکه برای سیستم عامل های لینوکسی مناسبه اما روی ویندوز معمولا بای دیففالت غیر فعال هست 

**nmap -PP (ICMP STAMP TIME SCAN ) : همون دستور پینگه ولی چون خیلی از فایروال ها پینگ رو میبندن , بجای اینکه پینگ ساده کنه و منتظر جواب بمونه , از سیستم ساعت رو میپرسه و بعضی از فایروال ها این دستور رو نمیبندن و فقط پینگ عادی رو حواسشون هست که ببندن ! زمان اسکنش هم یکم طولانی تره 

**nmap -PM (ICMP Address Mask Ping Scan ) : اگه توی متود های پینگ هیچکدوم از قبلی ها کار نکرد ممکنه این کار کنه . چرا که توی این متود سیستم یه درخواست خیلی قدیمی ارسال میکنه و از سیستم بعدی میپرسه ساب نت مسکت چیه و اون جواب میده . این متود قدیمیه چون با اومدن دی سی اج پی عملا به این درخواست نیاز نیست و مدیرای شبکه یادشون میره اونو ببندن 

**nmap -PS (TCP Syn Ping Scan ) : یکی از مهم ترین ابزار ها برای کشب سیستم در اینترنت . این متود یه پکت ساین میفرسته به یه پورت مثلا 80 و شروع یه دست دادن سه مرحله ای رغم میزنه و هاست یا میخواد ارتباط شکل بگیره و جواب میده یا میخواد شکل نگیره و بازم جواب میده و اینطوری میفهمیم هاست روشنه یا خیر . خیلی قدرتمنده و اگه متودای قبلی کار نکنن این به احتمال خیلی زیاد کار میکنه 


**nmap -PA (TCP Ack Ping Scan ) : همون سیستم بالاییه ولی به صورت سرزده به عنوان اولین فلگ , فلگ تاییدیه میفرسته و فایروال اجازه ورود میده و سیستم متوجه درسخواست نمیشه و یه فلگ ریست میفرسته ! ممکن همیشه جواب نده . تو تست خودم متود بالا روی یک سایت جوابگو بود اما این متود نه ! 

***namp -PO (Ip Protocol Ping Scan ) : این متود یه متود جالبه که میاد و هدر ایپی رو به پروتوکل های ناشناخته تغییر میده , یعنی نه دیگه پروتول ای سی ام پی و نه پروتوکل تی سی پی و یو دی پی ! میاد و پروتوکل های خاص رو میفرسته و فایروال ممکنه نحوه برخورد باهاشون براش تعریف نشده باشه و اینطوری اجازه ورود میده و سیستم یا میشناسه و یه پیام به خصوص میده و یا نمیشناسه و بازم یه پیام با عنوان پروتوکل ناشناخته میفرسته ! 
