...
**Objective:** This section explores techniques for evading firewalls and intrusion detection systems (IDS/IPS) during network scanning. These methods are essential for ethical hackers to test network defenses without immediate detection, allowing for a more comprehensive assessment of vulnerabilities.

**Strategic Approach:** Firewall and IDS evasion relies on manipulating packet structures, sources, and timing to bypass rule-based filters. Key techniques include fragmentation to split packets, source port manipulation to mimic legitimate traffic, IP decoys for obfuscation, custom packet creation to alter signatures, and bad checksums to probe system weaknesses. For anonymity, tools like proxies, proxy chaining, and Tor hide the attacker's origin. Finally, network topology mapping tools visualize the scanned environment, aiding in strategic planning and reporting.

A skilled penetration tester combines these methods with passive reconnaissance to minimize noise, ensuring stealth while mapping the attack surface. Always apply these in authorized environments to comply with ethical standards.

---

#### **B. Personal Deep Dive Notes (جزوه شخصی برای مرور)**

در این بخش، تکنیک‌های بای‌پس فایروال و IDS را با جزئیات، مثال‌ها و منطق فنی بررسی می‌کنم. این روش‌ها بیشتر برای فریب سیستم‌های قدیمی یا متوسط کاربرد دارند، اما در شبکه‌های مدرن اغلب با ترکیب تکنیک‌ها استفاده می‌شوند.

---

##### **۱. Packet Fragmentation (-f)**

> **برداشت من:** این تکنیک پکت‌ها را تکه‌تکه می‌کند تا از فایروال‌های قدیمی رد شود، اما سیستم‌های مدرن آن را بازسازی و تشخیص می‌دهند. بیشتر ارزش آموزشی دارد.

🧠 **یادآوری با مثال (Analogy):**  
تکنیک "ارسال نامه در چند پاکت جداگانه". نگهبان (فایروال قدیمی) هر پاکت کوچک را جداگانه چک می‌کند و چون تهدید کامل در هیچ‌کدام نیست، اجازه عبور می‌دهد. اما سیستم مقصد پاکت‌ها را مانند پازل سر هم می‌کند.

⚙️ **منطق فنی و استراتژی:**  
Nmap پکت‌ها را به قطعات کوچک (۸ بایت با -f یا ۱۶ بایت با -ff) تقسیم می‌کند تا از بررسی کامل توسط IDSهای ساده جلوگیری کند. فایروال‌های قدیمی فقط قطعه اول را چک می‌کنند، اما مدرن‌ها (با Fragment Reassembly) همه قطعات را نگه می‌دارند و قبل از عبور، بازسازی می‌کنند. استراتژی: برای CTFها یا شبکه‌های legacy مفید است، اما در دنیای واقعی برای ایجاد بار اضافی روی سیستم استفاده می‌شود.

⚡️ **دستور اصلی (Command):**  
`sudo nmap -f scanme.nmap.org` (برای قطعات ۸ بایتی)  
`sudo nmap -ff scanme.nmap.org` (برای قطعات ۱۶ بایتی)

---

##### **۲. Source Port Manipulation (-g / --source-port)**

> **برداشت من:** این روش پورت منبع را تغییر می‌دهد تا شبیه ترافیک合法 (مثل DNS) به نظر برسد، اما فایروال‌های Stateful آن را تشخیص می‌دهند.

🧠 **یادآوری با مثال:**  
تکنیک "وانمود کردن به پستچی DNS". فایروال ساده فکر می‌کند این یک پاسخ DNS معمولی است و اجازه عبور می‌دهد، اما فایروال هوشمند چک می‌کند که آیا درخواستی قبلی وجود داشته یا نه.

⚙️ **منطق فنی و استراتژی:**  
Nmap به طور پیش‌فرض از پورت‌های random و غیرعادی استفاده می‌کند که ممکن است بلاک شود. با -g، پورت منبع را به پورت‌های مجاز (مثل ۵۳ برای DNS یا ۸۰ برای HTTP) تغییر می‌دهیم. فایروال‌های ساده رد می‌کنند، اما Statefulها جلسه (session) را چک می‌کنند و اگر بخشی از اتصال موجود نباشد، بلاک می‌کنند. استراتژی: برای عبور از قوانین خاص پورت مفید است، اما با تکنیک‌های دیگر ترکیب شود.

⚡️ **دستور اصلی (Command):**  
`sudo nmap -g 53 scanme.nmap.org` (تغییر پورت منبع به ۵۳)

---

##### **۳. IP Decoy Scanning (-D)**

> **برداشت من:** این تکنیک برای مخفی‌کاری بالا عالی است، چون اسکن را از چندین IP جعلی پخش می‌کند و شناسایی منبع واقعی را سخت می‌کند.

🧠 **یادآوری با مثال:**  
تکنیک "ارتش سایه‌ها". تو یکی از سربازهای واقعی هستی، اما فایروال اسکن را از ده‌ها IP مختلف می‌بیند و نمی‌تواند تشخیص دهد کدام واقعی است – لوگ‌ها پر می‌شود و تحلیل سخت‌تر.

⚙️ **منطق فنی و استراتژی:**  
Nmap IPهای جعلی (decoys) را spoof می‌کند و اسکن را شبیه‌سازی می‌کند تا انگار از چندین منبع می‌آید (IP واقعی در میان آن‌هاست). سیستم هدف به همه پاسخ می‌دهد، اما نتایج فقط به IP واقعی برمی‌گردد. استراتژی: برای ایجاد نویز و سردرگمی در لوگ‌ها مفید است، اما فایروال‌های مدرن ممکن است الگوها را تشخیص دهند. از RND برای IPهای random یا ME برای قرار دادن IP خود در لیست استفاده کنید.

⚡️ **دستور اصلی (Command):**  
`sudo nmap -D RND:10 scanme.nmap.org` (۱۰ IP جعلی random)  
`sudo nmap -D 1.2.3.4,ME,5.6.7.8 scanme.nmap.org` (با IPهای خاص و ME)

---

##### **۴. Creating Custom Packets by Appending String/Binary Data (--data-string / --data / --data-length)**

> **برداشت من:** این روش پی‌لود پکت را افزایش می‌دهد تا امضای اسکن تغییر کند و IDSهای ساده نتوانند آن را تشخیص دهند، اما بزرگ کردن بیش از حد ممکن است پکت را دراپ کند.

🧠 **یادآوری با مثال:**  
تکنیک "بار اضافی به بسته پستی اضافه کن". به جای پکت خالی و مشکوک، داده‌های custom اضافه می‌کنی تا شبیه ترافیک واقعی (مثل درخواست وب) شود و نگهبان گیج شود.

⚙️ **منطق فنی و استراتژی:**  
Nmap داده‌های string (ASCII) یا binary (hex) را به پکت append می‌کند تا signature حمله را تغییر دهد. --data-length داده random اضافه می‌کند. استراتژی: برای evasion از IDSهای rule-based مفید است، اما اگر پی‌لود خیلی بزرگ شود، ممکن است MTU را بشکند و پکت fragment شود یا drop گردد. با تکنیک‌های دیگر ترکیب کنید.

⚡️ **دستور اصلی (Command):**  
`sudo nmap --data-string "TestPayload" -sS scanme.nmap.org` (اضافه کردن string)  
`sudo nmap --data 0xdeadbeef -sS scanme.nmap.org` (اضافه کردن hex binary)  
`sudo nmap --data-length 50 -sS scanme.nmap.org` (اضافه کردن ۵۰ بایت random)

---

##### **۵. Bad Checksum (--badsum)**

> **برداشت من:** این تکنیک برای تست قدرت فایروال است – چک می‌کند آیا چک‌سام را بررسی می‌کند یا نه، و برای شناسایی ضعف‌ها مفید است.

🧠 **یادآوری با مثال:**  
تکنیک "کد امنیتی اشتباه بنویس". فایروال هوشمند پکت را دراپ می‌کند، اما ضعیف ممکن است رد کند یا اصلاح کند – مثل نگهبانی که چک نمی‌کند!

⚙️ **منطق فنی و استراتژی:**  
Nmap چک‌سام پکت‌ها (IP/TCP/UDP) را عمداً اشتباه می‌فرستد. سیستم‌های استاندارد drop می‌کنند، اما فایروال‌های ضعیف ممکن است پاسخ دهند. استراتژی: اگر پاسخی آمد، یعنی ضعفی وجود دارد (چک‌سام چک نمی‌شود) – برای پروبینگ اولیه مفید، اما در مدرن‌ها بی‌اثر است.

⚡️ **دستور اصلی (Command):**  
`sudo nmap --badsum -sS scanme.nmap.org`

---

##### **۶. Staying Hidden with Proxy, Proxy Chaining, and Tor**

> **برداشت من:** این روش‌ها برای anonymity در اسکن‌ها ضروری‌اند – پراکسی ساده، chaining لایه‌ای، و Tor پیشرفته‌ترین برای پنهان کردن IP واقعی.

🧠 **یادآوری با مثال:**  
پراکسی: "از پشت یک دیوار جاسوسی کن". chaining: "از زنجیره دیوارها". Tor: "در شبکه پیازی گم شو" – لایه‌لایه رمزنگاری شده.

⚙️ **منطق فنی و استراتژی:**  
- **Proxy Server:** درخواست‌ها را از IP پراکسی فوروارد می‌کند (HTTP/SOCKS).  
- **Proxy Chaining:** زنجیره پراکسی‌ها برای لایه بیشتر anonymity.  
- **Tor:** از relayها برای onion routing استفاده می‌کند، IP را کامل پنهان می‌کند اما کند است. استراتژی: برای اسکن‌های خارجی عالی، اما در داخلی با VPN ترکیب شود. ابزار: Proxychains یا torsocks.

⚡️ **دستور اصلی (Command):**  
`proxychains nmap -sS scanme.nmap.org` (برای chaining)  
`torsocks nmap -sS scanme.nmap.org` (برای Tor)

---

##### **۷. Drawing Network Topology**

> **برداشت من:** بعد از اسکن، ترسیم گرافیکی topology کمک می‌کند شبکه را بهتر بفهمیم و نقاط ضعف را ببینیم. ابزارهای مختلفی برای این کار وجود دارند.

🧠 **یادآوری با مثال:**  
تکنیک "نقشه شهر را بکش". این ابزارها مثل نقشه‌کش‌های خودکار عمل می‌کنند که ساختمان‌ها (دستگاه‌ها) و جاده‌ها (لینک‌ها) را نشان می‌دهند.

⚙️ **منطق فنی و استراتژی:**  
این ابزارها با discovery خودکار (SNMP/ICMP) شبکه را مپ می‌کنند و دیاگرام‌های interactive می‌سازند. استراتژی: داده‌های Nmap را ایمپورت کنید تا topology لایه ۲/۳ را visualize کنید – برای گزارش‌نویسی و برنامه‌ریزی حمله مفید است.

**ابزارهای پیشنهادی:**  
- **Network Topology Mapper (SolarWinds):** Auto-discovery سریع، mapping L2/L3، export آسان.  
- **OpManager (ManageEngine):** Monitoring + mapping، alerting، cloud support.  
- **NetBrain:** Automation برای شبکه‌های بزرگ، dynamic maps، path analysis.  
- **Maltego:** Graph-based برای OSINT و relationships، extensible با transforms.  

| ابزار                  | بهترین برای          | مزایا کلیدی                  | معایب کلیدی              |
|------------------------|----------------------|-------------------------------|---------------------------|
| Network Topology Mapper | اسکن ساده           | Auto-discovery سریع، export آسان | محدود برای بزرگ‌ها      |
| OpManager             | Monitoring + Mapping | Alerting، cloud support      | پیچیده برای شروع        |
| NetBrain              | Enterprise بزرگ     | Automation، dynamic maps     | گران، نیاز به expertise |
| Maltego               | OSINT و graphs      | Graph-based، extensible      | کمتر فیزیکی، یادگیری سخت
