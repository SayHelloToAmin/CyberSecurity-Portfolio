...
**nmap -sI (Idle Scan) : این یکی از مخفی ترین روش هاست چون ما از یه سیستم واسطه استفاده میکنیم و پورت رو اسکن میکنیم . توی اینترنت هر پکتی که سیستم ارسال میکنه یه ایدی داره و هربار که سیستم یه پکت ارسال میکنه یه دونه به این ایدی اضافه میشه . توی این روش ما به سیستم زامبی یه پکت با فلگ ساین اکنالج میفرستیم و اون ریست میکنه و ایدیش رو میگیریم . حالا به سیستم هدف با ایپی فیک زامبی یه پکت ساین میفرستیم و اگر پورت باز باشه سیستم هدف به سیستم زامبی یه پکت ساین اک میفرسته و زامبی اون رو ریست میکنه و پکت ایدی یکی افزایش پیدا میکنه و دوباره ایدی جدید رو از زامبی میگیریم و اختلافشون بهمون نشون میدن چه اتفاقی افتاده 

**nmap -sU (UDP Scan): این اسکن از پروتوکل یو دی پی استفاه میکنه ! سیستم اینطوریه که یه پکت به هدف میفرسته و اگر پیغام پورت انریچبل گرفت یعنی پورت بستست اما درباره باز بودنش ابهام وجود داره که ایا باز بود و سکوت کرد یا فایروال جلوش رو گرفت ! مزیتش اینه که تنها راه اسکن بعضی از پورتای خاص یو دی پی هست و بدیش هم کند بودنشه چون پاسخی دریافت نمیکنه باید یه تایمیو خود ان مپ بگذرونه و بعد نتیجرو نشون بده 
